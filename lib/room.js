...
var getRoom = Promise.promisify(function (id, cb) {
    request({
...
var enrichLocation = Promise.promisify(function (room, cb) {
...
Parser.prototype._parseUserImage = function ($userImage) {
    return {
        host_id: path.basename($userImage.find('a').attr('href')),
        host_name: $userImage.find('a img').attr('title')
    };
};
